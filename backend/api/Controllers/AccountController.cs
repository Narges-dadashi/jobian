...
public class AccountController(IAccountRepository accountRepository) : BaseApiController
{
    ...
    public async Task<ActionResult<LoggedInDto>> RegisterJobSeeker(JobSeekerRegisterDto userInput, CancellationToken cancellationToken)
    {
        ...
        OperationResult<LoggedInDto> opResult = await accountRepository.RegisterJobSeekerAsync(userInput, cancellationToken);
    ...
    public async Task<ActionResult<LoggedInDto>> RegisterEmployer(EmployerRegisterDto userInput, CancellationToken cancellationToken)
    {
        ...
        OperationResult<LoggedInDto> opResult = await accountRepository.RegisterEmployerAsync(userInput, cancellationToken);
    ...
    public async Task<ActionResult<LoggedInDto>> ReloadLoggedInUser(CancellationToken cancellationToken)
    {
        ...
        if (string.IsNullOrEmpty(token))
            return Unauthorized("Token is expired or invalid. logged in again!");
        ...
        return loggedInDto is null ? Unauthorized("User is logged out or unauthorize. Logged in again!") : loggedInDto;
    }
}